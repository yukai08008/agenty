...
- v0.05-d 已完成自动回归并提交，等待用户验收。
- 下一步按回归基线进入 v0.05-e 交互管理；开始前应以 OpenCode 1.18.26 的通道能力重新确认 ask/auto 的边界。
- 仓库级交接入口为 `AGENTS.md`；其他 Agent 应先按其中顺序恢复 Git 与 PM 状态。

## 当前工作区注意事项

- 当前功能基线：`v0.05-d@732df2f`。
- `pyproject.toml` 和 `uv.lock` 存在用户为 IPython 添加依赖产生的未提交修改；后续 Agent 必须保留，除非用户明确决定如何处理。
- v0.05-d 已完成默认自动测试，尚未记录用户 IPython 手动验收结论，也没有重新执行真实模型 live 测试。
