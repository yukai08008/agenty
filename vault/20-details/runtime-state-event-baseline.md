...
该基线仍可在版本内调整。v0.01-b 实现 Availability 状态转移和 Capability；v0.01-c 实现通用 Probe 编排与 OpenCode 1.18.26 无副作用 Adapter，并将失败上下文写入 RuntimeSnapshot。v0.03 已实现公共 TurnMachine 和 OpenCode Turn 调用；v0.05-b 增加连续 sequence、上下文锁定及 raw 诊断隔离的 EventStream。Channel 与 Session 行为仍未实现。
