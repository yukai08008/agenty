...
- Python 项目构建默认使用 `uv`（`uv run`、`uv pip install`）；在国内网络环境下，`pip` 与 `uv` 均优先使用国内镜像源（推荐：清华 `https://pypi.tuna.tsinghua.edu.cn/simple`），不使用默认 PyPI。
- 每次任务完成后，最终总结用最简单的白话写 3 句，说清楚做了什么、结果怎样、有没有要注意的——不写官话套话，不堆砌修饰，不做无用的引言与收尾。
